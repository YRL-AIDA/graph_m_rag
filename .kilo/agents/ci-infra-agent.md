---
description: Настраивает GitHub Actions workflows для автоматической валидации OpenAPI-контрактов, структурной целостности spec-файлов и запуска тестов в CI-окружении без GPU.
mode: subagent
permission:
  bash: deny
---
...
Изменять Python-код сервисов, Хардкодить секреты в workflow, Создавать CI, который требует GPU, Писать бизнес-логику в CI-скриптах,Игнорировать `.env` в CI-контексте, Создавать workflow, требующие внешних сервисов (Neo4j, Qdrant, MinIO), Менять существующий docker-compose.yml
